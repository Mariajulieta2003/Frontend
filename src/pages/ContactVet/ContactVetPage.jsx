...
import { apiCreateVetConsult } from "../../api/vetConsults.js";
...
export default function ContactVetPage() {
  ...
  // ⬇️ Ahora envía la consulta al backend
    const handleSubmit = async (e) => {
      e.preventDefault();
      setSubmitting(true);

      try {
        await apiCreateVetConsult({
          petId: form.petId === "other" ? null : form.petId,
        ...
        alert("No pudimos enviar la consulta. Intentá nuevamente.");
      } finally {
        setSubmitting(false);
      }
    };
  ...
  return (
    <main className="cvp-root">
      {/* ⬆️ ENCABEZADO CON BOTÓN ARRIBA A LA DERECHA */}
      <section className="cvp-header">
        ...
        <Link to="/vet/history" className="cvp-historyBtn">
          Ver historial 🩺
        </Link>
      </section>
