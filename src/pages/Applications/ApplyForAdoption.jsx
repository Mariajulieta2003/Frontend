// src/pages/Applications/ApplyForAdoption.jsx
...
export default function ApplyForAdoption() {
  ...
  // Prefill básico con datos del usuario + cargar borrador
  useEffect(() => {
    setForm((f) => ({
      ...f,
      fullName: f.fullName || currentUser?.name || currentUser?.full_name || "",
  ...
  }, [currentUser?.name, currentUser?.full_name, currentUser?.email, currentUser?.phone, currentUser?.city]);
  ...
  // Redirección automática cuando success = true
  useEffect(() => {
    if (!success) return;
    const t = setTimeout(() => navigate("/pets"), 3000);
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      setServerError(err?.response?.data?.message || "No pudimos enviar la postulación.");
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <form className="apply-form" onSubmit={onSubmit} noValidate>
          ...
          <div className="f-row">
            <div className="f-col">
              ...
              <select
                ...
                onChange={(e) => update("hasOtherPets", e.target.value)}
                required
              >
            ...
            <div className="f-col">
              ...
              <select
                value={form.hasKids}
                onChange={(e) => update("hasKids", e.target.value)}
                required
              >
                <option value="">Seleccioná…</option>
                <option value="si">Sí</option>
          ...
          <div className="f-row">
            ...
            <div className="f-col">
              ...
              <select
                ...
                onChange={(e) => update("outdoorSpace", e.target.value)}
                required
              >
                <option value="">Seleccioná…</option>
                <option value="si">Sí</option>
          ...
          <div className="f-row">
            <div className="f-col">
              ...
              <select
                value={form.commitmentNeuter}
                onChange={(e) => update("commitmentNeuter", e.target.value)}
                required
              >
                <option value="">Seleccioná…</option>
                <option value="si">Sí</option>
          ...
          <label className="f-accept">
            ...
            />
            <span>
              Acepto el uso de mis datos de contacto para gestionar esta postulación.
            </span>
          </label>
          ...
          <div className="apply-card">
            ...
            <div className="apply-pet">
              ...
              <div className="apply-pet-info">
                ...
                <div className="apply-pet-meta">
                  {[pet.city, pet.state, pet.province].filter(Boolean).join(", ")}
                </div>
              </div>
            </div>
            <Link to={`/pets/${id}`} className="apply-btn link">
              Ver publicación
            </Link>
          </div>
        </aside>
